...
The workflow skips scheduled runs with a summary while `DevSnapshot__Ready` is false; a manual run fails early in that state. Repository variable `BITWARDEN_DEV_AUTH_E2E_SECRETS` maps only the existing `DEV_SNAPSHOT_MEMBER_PASSWORD` Bitwarden secret. A third-party provider callback still needs a dedicated provider test identity and a separate browser journey.
