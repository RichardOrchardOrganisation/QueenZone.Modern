# App Store metadata — English (Australia)

## App-level information

- **Name:** QueenZone
- **Primary language:** English (Australia)
- **Bundle ID:** `org.queenzone.mobile`
- **Suggested SKU:** `QZ-IOS-001` (internal and permanent after record creation)
- **Primary category:** Music
- **Secondary category:** Reference
- **Content rights:** The app contains and links to third-party Queen-related editorial and archival material. Confirm that QueenZone has the necessary rights or permissions for every item offered in the app before answering the App Store Connect declaration.

## Product page copy

- **Subtitle:** Queen archive and community
- **Promotional text:** Explore decades of Queen history, restored stories, photography and news—then join the QueenZone community from your iPhone.
- **Keywords:** Queen,Freddie Mercury,archive,music,rock,photography,discography,fan,forum,history

### Description

QueenZone brings a long-running independent Queen fan archive to iPhone.

Explore decades of Queen history through restored news, long-form stories, photography, biography, discography, timelines and fan performances. Search the archive, save material for later and keep favourite stories close at hand.

Stay current with newly published Queen news, browse carefully preserved historical material and explore thousands of photographs organised into collections.

Members can take part in the QueenZone community: read and join forum discussions, exchange private messages, suggest news, submit photographs directly from the camera or photo library, and choose which notifications they receive.

Native features include:

• Offline-friendly reading for previously loaded content
• Push notifications for selected community activity and news
• Camera and photo-library submission
• Home Screen “On This Day” widget
• Sharing news links into QueenZone
• Light and dark appearance, text-size preferences and reduced-motion support

QueenZone is an independent, fan-run archive and community. It is not affiliated with Queen, its members, management, record labels or representatives.

## URLs

- **Support URL:** `https://www.queenzone.org/contact`
- **Marketing URL:** `https://www.queenzone.org`
- **Privacy Policy URL:** `https://www.queenzone.org/privacy`
- **Privacy Choices URL:** `https://www.queenzone.org/data-deletion`

## Version information

- **Suggested first public version:** `1.0`
- **What's New:** Not required for the first version.
- **Release option:** Manually release this version.
- **Copyright:** `© 2026 [LEGAL SELLER NAME]` — replace the placeholder with the exact Apple Developer legal entity or authorised trading name.

## Availability and pricing

- **Price:** Free
- **Territories:** All territories unless content-rights review identifies restrictions.
- **Pre-order:** No

